...
contract MarketPlace {
    ...
    function ListItems(string memory _itemName, string memory _itemDescription, uint256 _itemPrice, uint256 _id, uint256 _quantity) external {
        if(msg.sender == address(0)) revert("cant be address zero");
        // if(msg.sender != owner) revert("Only owner can perform action");
        Items storage it = items[_id];
